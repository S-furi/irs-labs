...
function vector.to_differential_steering(robot, v)
	local max_velocity = 15
	L = robot.wheels.axis_length
	local v1, w = v.length, v.angle
	local left_velocity = (v1 * max_velocity) - (L  * w) / 2
	local right_velocity = (v1 * max_velocity) + (L  * w) / 2
	return math.min(left_velocity, max_velocity), math.min(right_velocity, max_velocity)
end
